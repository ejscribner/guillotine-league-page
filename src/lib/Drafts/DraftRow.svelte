...
<Row>
  {#each draftRow as draftCol, col}
    {#if !previous || draftCol}
      <Cell
        class="draftCell{draftCol ? ' changedHands' : ''}{previous
          ? ` prev${players[draftCol.player].pos}`
          : ''}"
      >
        <span class="draftPos{previous ? 'Prev' : ''}">
          ...
          {:else if draftType == "snake" && reversalRound}
            ...
            {draftCol?.newOwner
              ...
              : ""}
          {:else if !reversalRound || row < reversalRound}
            {row}.{col + 1}{draftCol?.newOwner
              ? ` ${getTeamNameFromTeamManagers(
                  leagueTeamManagers,
                  draftCol.newOwner,
                  year
              ...
              : ""}
          {:else}
            {row}.{draftRow.length - col}{draftCol?.newOwner
              ? ` ${getTeamNameFromTeamManagers(
                  leagueTeamManagers,
                  draftCol.newOwner,
                  year
        ...
        {#if draftCol && !previous}
          <div class="newOwner">
            {getTeamNameFromTeamManagers(leagueTeamManagers, draftCol)}
          </div>
        {/if}
        {#if previous}
          <div
          ...
          <br />
          <div class="name">
            {`${players[draftCol.player].fn} ${
              players[draftCol.player].ln
            }`}{players[draftCol.player].pos == "DEF"
              ? ""
              : ` (${players[draftCol.player].t})`}
          </div>
        {/if}
      </Cell>
    {/if}
