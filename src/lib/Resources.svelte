<script>
  ...
  const resources = [
    ...
    },
    {
      ...
      url: `https://www.fantasypros.com/${today.getFullYear()}/${
        today.getMonth() + 1
      }/fantasy-football-rankings-dynasty-trade-value-chart-${today
        .toLocaleString("default", { month: "long" })
        .toLowerCase()}-${today.getFullYear()}-update/`,
      icon: "assessment",
      premium: false,
      separator: true,
      dynastyOnly: true,
      ...
      {#if resource.dynastyOnly && dynasty}
        <a target="_blank" href={resource.url}>
          <Item>
            ...
            {/if}
            <Text class="linkText"
              >{resource.name}{resource.premium ? "*" : ""}</Text
            >
          </Item>
...
<style>
  ...
  :global(.list) {
    width: 90%;
    max-width: 800px;
    border: 1px solid var(--mdc-theme-text-hint-on-background, var(--d7d7d7));
    margin: 15px auto;
    padding: 0 !important;
