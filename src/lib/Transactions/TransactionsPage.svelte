<script>
  ...
  const setQuery = (query, filteredTransactions) => {
    ...
    if (query && query.trim() != "") {
      subsetTransactions = filteredTransactions.filter((transaction) =>
        checkForQuery(transaction)
      );
  ...
  const changePage = (dest, pageChange = false) => {
    ...
    displayTransactions = setQuery(query, filteredTransactions);
    if (postUpdate) {
      goto(`/transactions?show=${show}&query=${query}&page=${page + 1}`, {
        noscroll: true,
        keepfocus: true,
      });
    }
  ...
  const search = () => {
    ...
    page = 0;
    if (postUpdate) {
      const dest = `/transactions?show=${show}&query=${query.trim()}&page=${
        page + 1
      }`;
      debounce(dest);
    }
  ...
  const clearSearch = () => {
    query = "";
    if (postUpdate) {
      goto(`/transactions?show=${show}&query=&page=${page + 1}`, {
        noscroll: true,
        keepfocus: true,
      });
    }
...
<div class="transactionsParent">
  ...
  <div class="searchContainer">
    ...
    {#if query.length > 0}
      <IconButton class="material-icons" on:click={() => clearSearch()}
        >clear</IconButton
      >
    {:else}
      <span class="clearPlaceholder" />
