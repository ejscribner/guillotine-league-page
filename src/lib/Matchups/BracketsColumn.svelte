    ...
    for (const k in startersWeeks) {
      ...
      for (const starter of starters) {
        totalPoints += parseFloat(
          players[starter]?.wi && players[starter].wi[playoffsStart - i]?.p
            ? players[starter].wi[playoffsStart - i].p
            : 0
        );
      }
    }
...
<div class="bracketColumn" bind:this={el}>
  ...
  {#each matchCol as matchups, inx}
    <div
      class="match{matchups[0].m == selected ? ' selected' : ''}{matchups[0]
        .m && matchups[1].roster_id
        ? ' clickable'
        : ''}"
      bind:this={anchors[Math.floor(inx / 2)][inx % 2 == 0 ? "t" : "b"]}
      on:click={() => {
        changeSelection(matchups[0].m, matchups[1].roster_id);
      }}
    >
        ...
        <div class="manager">
          <div class="avatarPointsBlock">
            ...
            {#if matchup.roster_id || (!matchups.bye && !matchup.roster_id)}
              <img
                class="avatar{!matchups.bye && !matchup.roster_id
                  ? ' avatarBye'
                  : ''}"
                src={getAvatarFromTeamManagers(
                  leagueTeamManagers,
                  matchup.roster_id,
                  leagueTeamManagers.currentYear
                )}
                alt="team avatar"
              />
            {/if}
            {#if matchup.roster_id}
              <div class="points">
                <div class="actualPoints">
                  {calculatePoints(matchup.points)}
                </div>
                <div class="projectedPoints">
                  {calculatePotentialPoints(matchup.starters, ix, players)}
                </div>
              </div>
            {:else}
              <span />
