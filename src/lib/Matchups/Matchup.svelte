<script>
  ...
  const digestStarters = (x, p) => {
    home = matchup[0];
    away = matchup[1];
    home.manager = getTeamFromTeamManagers(
      leagueTeamManagers,
      home.roster_id,
      year
    );
    away.manager = getTeamFromTeamManagers(
      leagueTeamManagers,
      away.roster_id,
      year
    );
    const homeStarters = matchupWeek
      ? home.starters[matchupWeek]
      : home.starters ?? [];
    if (homeStarters.length === 0) {
      home.starters = [];
    }
    const awayStarters = matchupWeek
      ? away.starters[matchupWeek]
      : away.starters ?? [];
  ...
  const digestStarter = (starter, points) => {
    ...
    return {
      ...
      team: player.t,
      opponent:
        player.wi && player.wi[displayWeek] ? player.wi[displayWeek].o : null,
      projection,
      points,
    };
  };
  ...
  const calcHeight = () => {
    ...
    const startersLength = matchupWeek
      ? home.starters[matchupWeek].length
...
<div class="matchup">
  <div class="header" on:click={() => expandClose()} bind:this={el}>
    ...
    <div class="opponent away{winning == 'away' ? ' awayGlow' : ''}">
      <div class="totalPoints totalPointsL">
  ...
  <div
    class="rosters"
    style="max-height: {active == ix ? calcHeight() + 'px' : '0'}; {active != ix
      ? 'border: none'
      : ''};"
  >
