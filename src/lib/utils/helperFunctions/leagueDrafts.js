...
const buildFromScratch = (
  ...
  if (progression == 0) {
    ...
    const sortedRosterKeys = rosterKeys.sort((a, b) => {
      ...
      return (
        rosterA.fpts +
        rosterA.fpts_decimal / 100 -
        (rosterB.fpts + rosterB.fpts_decimal / 100)
      );
    });
...
const completedNonAuction = ({ players, draft, picks, draftOrder, rounds }) => {
  ...
  for (const pick of picks) {
    if (pick.owner_id == pick.roster_id || pick.round > rounds) continue;
    draft[pick.round - 1][draftOrder.indexOf(pick.roster_id)].newOwner =
      pick.owner_id;
  }
  return draft;
};
...
export const getPreviousDrafts = async () => {
  ...
  while (curSeason && curSeason != 0) {
    const [leagueData, completedDraftsInfo] = await waitForAll(
      getLeagueData(curSeason).catch((err) => {
        console.error(err);
      }),
