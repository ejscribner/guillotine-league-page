...
export const getBrackets = async (queryLeagueID = leagueID) => {
  ...
  const bracketsAndMatchupResps = await waitForAll(
    ...bracketsAndMatchupFetches
  ).catch((err) => {
    console.error(err);
  });
  ...
  const playoffMatchups = await waitForAll(...bracketsAndMatchupJson).catch(
    (err) => {
      console.error(err);
    }
  );
...
const evaluateBracket = (contestants, rounds, playoffMatchups, playoffType) => {
  ...
  for (let i = 1; i <= rounds; i++) {
    ...
    for (const playoffBracket of playoffBrackets) {
      if (
        (!playoffBracket.t1_from && playoffBracket.t2_from) ||
        (!teamsSeen[playoffBracket.t1] && teamsSeen[playoffBracket.t2])
      ) {
        // this was from a team that got a bye
        let byeMatchup = processPlayoffMatchup({
          playoffBracket,
          playoffMatchups,
          i: i - 2,
          consolationMs,
      ...
      const roundMatchup = processPlayoffMatchup({
        playoffBracket,
        playoffMatchups,
        i: i - 1,
        consolationMs,
    ...
    for (const consolation of consolations) {
      for (const consolationMatchup of consolationMatchups) {
        ...
        if (
          consolationMatchup[0].winners &&
          consolation[i - 2] &&
          consolation[i - 2] &&
          consolationMatchup[0].t1From == consolation[i - 2][0][0].m
        ) {
    ...
    const notFromWinners = consolationMatchups.filter(
      (m) => !m[0].fromWinners && !m[0].winners
    );
    // These are matchups between teams that lost in the championship bracket
    const fromWinners = consolationMatchups.filter(
      (m) => m[0].fromWinners && !m[0].winners
    );
...
const processPlayoffMatchup = ({
  ...
  const winners =
    playoffBracket.t1_from?.w && playoffBracket.t2_from?.w ? true : false;
  ...
  if (
    (p && p != 1) ||
    (playoffBracket.t1_from?.l && playoffBracket.t2_from?.l) ||
    consolationMs.indexOf(t1From) > -1 ||
    consolationMs.indexOf(t2From) > -1
  ) {
    consolation = true;
  }
...
const generateMatchupData = (
  ...
  if (t) {
    ...
    if (playoffType == 2 || (p && p == 1 && playoffType == 1)) {
      const secondWeek = playoffMatchups[i + 1].filter(
        (ma) => ma.roster_id == t
      )[0];
