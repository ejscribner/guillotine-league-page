  ...
  const computePages = (curPage, pages, iW) => {
    ...
    for (let i = 0; i < pages; i++) {
      if (
        i == 0 ||
        (i == pages - 1 && (!iW || iW > 300)) ||
        (curPage - limit < i && i < curPage + limit)
      ) {
...
{#if total > 0 && totPages > 1}
  <div class="paginationBar">
    {#if page > 0}
      <Icon class="material-icons button" on:click={() => changePage(page - 1)}
        >chevron_left</Icon
      >
    {:else}
    ...
    <div class="numbers">
      {#each pageLabels as pageLabel}
        ...
        {:else}
          <span class="dest pg" on:click={() => changePage(pageLabel - 1)}
            >{pageLabel}</span
          >
        {/if}
      {/each}
  ...
  <div class="totals">
    {page * perPage + 1} - {page + 1 == totPages ? total : (page + 1) * perPage}
    of {total}
  </div>
{/if}
