  ...
  {#each comments as comment}
    <div class="comment">
      <img
        alt="author avatar"
        class="teamAvatar"
        src={getAvatar(leagueTeamManagers, comment.fields.author)}
      />
      <span class="author"
        >{@html getAuthor(leagueTeamManagers, comment.fields.author)} -
      </span>
      <span class="commentText">{@html comment.fields.comment}</span>
      <div class="date"><i>{parseDate(comment.sys.createdAt)}</i></div>
    </div>
  {/each}
  <CreateComment bind:showWrite on:createComment={addComment} />
</div>
