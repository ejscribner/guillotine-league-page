<script>
  ...
  const setData = (lRR) => {
    ...
    for (const seasonWeekRecord of seasonWeekRecords) {
      yearsObj[seasonWeekRecord.year].weekRecords =
        seasonWeekRecord.seasonPointsHighs;
      yearsObj[seasonWeekRecord.year].weekLows =
        seasonWeekRecord.seasonPointsLows;
      yearsObj[seasonWeekRecord.year].blowouts =
        seasonWeekRecord.biggestBlowouts;
      yearsObj[seasonWeekRecord.year].closestMatchups =
        seasonWeekRecord.closestMatchups;
    }
    ...
    for (const rosterID in lRR) {
      ...
      for (const season of leagueManagerRecord.years) {
        ...
        yearsObj[season.year].winPercentages.push({
          rosterID,
          percentage: round(
            ((season.wins + season.ties / 2) /
              (season.wins + season.ties + season.losses)) *
              100
          ),
          wins: season.wins,
          ties: season.ties,
    ...
    for (const key in yearsObj) {
      // sort records
      yearsObj[key].seasonLongLows = yearsObj[key].seasonLongRecords
        .slice()
        .sort((a, b) => a.fpts - b.fpts)
        .slice(0, 10);
      yearsObj[key].seasonLongRecords = yearsObj[key].seasonLongRecords
  ...
  <Group variant="outlined">
    {#each years as { year }, ix}
      <Button
        ...
        on:click={() => (display = ix)}
        variant={display == ix ? "raised" : "outlined"}
      >
        <Label>{year}</Label>
      </Button>
    {/each}
