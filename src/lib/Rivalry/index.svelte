<script>
  ...
  const setTradeHistory = (p1, p2) => {
    ...
    return trades.map((t) => {
      const rosterIDOne = parseInt(
        getRosterIDFromManagerIDAndYear(leagueTeamManagers, playerOne, t.season)
      );
      const rosterIDTwo = parseInt(
        getRosterIDFromManagerIDAndYear(leagueTeamManagers, playerTwo, t.season)
      );
  ...
  $: playerOneRecords = recordsInfo?.regularSeasonData?.leagueManagerRecords
    ? recordsInfo.regularSeasonData.leagueManagerRecords[playerOne]
    : null;
  $: playerTwoRecords = recordsInfo?.regularSeasonData?.leagueManagerRecords
...
<div class="rivalrySelection">
  <ManagerSelectors bind:playerOne bind:playerTwo {leagueTeamManagers} />
</div>
